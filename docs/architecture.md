...
- サーバーサイド（Python）でコミットの座標を計算し、Jinja2 テンプレートで SVG を生成する
- 座標系は碁盤の目（グリッド）。各コミットを `(列, 行)` のセルにひとつだけ配置する
- エッジ（親子関係）は SVG の `<path>` 要素で描画する
- D3.js は使用しない
- アルゴリズムの詳細は `docs/graph-algorithm.md` を参照する
